...
<div class="bg-white shadow-md rounded-lg p-6 max-w-md mx-auto">
    ...
    <div class="mt-4 flex space-x-2">
        <a href="{{ route('salaries.edit', $salary->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-400">Edit</a>
        <form action="{{ route('salaries.destroy', $salary->id) }}" method="POST" onsubmit="return confirm('Apakah yakin ingin menghapus?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-500">Hapus</button>
        </form>
        <a href="{{ route('salaries.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-400">Kembali</a>
    </div>
</div>
