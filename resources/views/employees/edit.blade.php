...
<form action="{{ route('employees.update', $employee->id) }}" method="POST" class="space-y-4">
    ...
    <a href="{{ route('employees.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-400">Batal</a>
</form>

<script>
    // Optional: AJAX untuk load posisi sesuai department
    const departmentSelect = document.getElementById('department_id');
    const positionSelect = document.getElementById('position_id');

    departmentSelect.addEventListener('change', function() {
        const deptId = this.value;
        fetch(`/api/departments/${deptId}/positions`)
            .then(res => res.json())
            .then(data => {
